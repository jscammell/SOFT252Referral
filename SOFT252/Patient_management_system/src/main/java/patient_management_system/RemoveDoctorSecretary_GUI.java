...
public class RemoveDoctorSecretary_GUI extends javax.swing.JFrame {
    ...
    private void btnSubmitActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSubmitActionPerformed
        ...
        for (int i = 0; i < length; i++) {
        Object[] newDoctors = doctors.toArray();
        String userId = ((User.Doctor)newDoctors[k]).getUserId();
        String aPassword = ((User.Doctor)newDoctors[k]).getPassword();
        String first_Name = ((User.Doctor)newDoctors[k]).getFirst_Name();
        String last_Name = ((User.Doctor)newDoctors[k]).getLast_Name();
        int ratingsLength = ((User.Doctor)newDoctors[k]).getRatingsLength();
        ...
        int[] ratings = ((User.Doctor)newDoctors[p]).getRatings();
        out.write(String.valueOf(ratings[1]));
        out.newLine();
        }
        
        for (int p = 0; p < ratingsLength; p++) {
        String[] description = ((User.Doctor)newDoctors[p]).getDescription();        
